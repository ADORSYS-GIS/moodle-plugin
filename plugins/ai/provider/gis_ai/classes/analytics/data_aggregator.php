...
final class data_aggregator {
    ...
    public static function aggregate(array $filters = []): array {
        ...
        try {
            $totalRequests = (int)$DB->count_records_select('aiprovider_gis_ai_logs', $where, $params);
            $uniqueUsers = (int)($DB->get_field_sql("SELECT COUNT(DISTINCT userid) FROM {aiprovider_gis_ai_logs} WHERE $where", $params) ?? 0);
            $totalTokens = (int)($DB->get_field_select('aiprovider_gis_ai_logs', 'SUM(tokens)', $where, $params) ?? 0);
            $avgTokens = $totalRequests > 0 ? round($totalTokens / max(1, $totalRequests), 2) : 0.0;
            $successRate = (float)($DB->get_field_select('aiprovider_gis_ai_logs', 'AVG(status)', $where, $params) ? : 0.0);
            $successRate = round($successRate * 100, 2);

            $pagedrows = [];
            if ($limit > 0) {
                $sql = "SELECT id, userid, timestamp, prompt_hash, tokens, status, contextid, courseid
                          FROM {aiprovider_gis_ai_logs}
                         WHERE $where
